...
## python function
def gauss_seidel(U:  ti.ext_arr(), solver : ti.template(), b: ti.template(), x: ti.template(),\
                 itr: ti.i32, x_old: ti.template()):
    for iter in range(itr):
        print(iter)
        gauss_seidel_itr(U, b, x, x_old)
        # x = L \ b - U x_old
        xtemp = solver.solve(x) ## cannot be called in a taichi function: return self.solver.solve(b.to_numpy())
